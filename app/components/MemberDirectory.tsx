...
export function MemberDirectory({ member }: { member: GdgMember }) {
  ...
  return <main className="members-page">
    <nav className="member-nav"><a href="/" className="member-brand">GDG <span /> <em>ON CAMPUS<br />KU</em></a><div className="member-nav-actions"><a className="member-directory-link" href="/member">My pass</a><span>{member.email}</span><MemberSignOut /><SectionMenu /></div></nav>
    <section className="members-hero"><p className="member-eyebrow">GDG ON CAMPUS / KU</p><div><h1>Meet your<br /><i>builders.</i></h1><p>Find classmates working on similar ideas, studying for the same challenge, or looking for their next project collaborator.</p></div><aside><span>PUBLIC PROFILES</span><b>{members.length}</b><small>verified GDG KU builders</small></aside></section>
