...
export function BuddyMatch({ member }: { member: GdgMember }) {
  ...
  const [selectedBuilder, setSelectedBuilder] = useState<DirectoryMember | null>(null);
  ...
  const matches = useMemo(() => {
    ...
    return builders.map((builder) => ({ builder, shared: [...words(builder.interests)].filter((interest) => mineSet.has(interest)) })).filter(({ builder, shared }) => !term || [builder.displayName, builder.major, builder.interests, builder.leetCodeUsername, builder.discordUsername, builder.instagramUsername, ...shared].join(" ").toLowerCase().includes(term)).sort((a, b) => b.shared.length - a.shared.length || a.builder.displayName.localeCompare(b.builder.displayName));
  ...
  const connections = requests.filter((request) => request.status === "accepted").map((request) => {
    ...
    return { ...request, peer, peerName: request.fromUid === member.uid ? (peer?.displayName || "GDG KU buddy") : request.fromName };
  });
  ...
  async function copyDiscordUsername(username: string) {
    try {
      await navigator.clipboard.writeText(username);
      setNotice(`@${username} copied. Paste it into Discord to add your co-builder.`);
    } catch {
      setNotice(`Your co-builder's Discord username is @${username}.`);
    }
  }

  return <main className="buddy-page">
    <nav className="member-nav buddy-nav site-header"><a href="/" className="member-brand">GDG <span /> <em>ON CAMPUS<br />KU</em></a><div className="member-nav-actions"><MemberSignOut /></div><SectionMenu /></nav>
    ...
    <section className="buddy-match-shell"><header className="buddy-match-head"><div><p className="member-eyebrow">YOUR SUGGESTIONS</p><h2>People to<br /><i>start with.</i></h2></div><div className="buddy-profile-note"><b>{mine.trim() ? "Matched from your interests" : "Complete your profile for better matches"}</b><p>{mine.trim() ? "Shared interests are ranked first. Update them anytime from your member pass." : "Add interests such as AI, frontend, cloud, or interview prep to make matching useful."}</p><a href="/member#profile">Edit my interests →</a></div></header>
      ...
      {connections.length > 0 && <section className="buddy-connections"><header><span>MY CONNECTIONS</span><h3>{connections.length} builder {connections.length === 1 ? "buddy" : "buddies"}</h3></header><div>{connections.map((connection) => <article key={connection.id}><p>CONNECTED</p><h4>{connection.peerName}</h4><span>{connection.goal} · {connection.availability}</span><div className="buddy-connection-actions"><button type="button" onClick={() => void copyIntroduction(connection.peerName, connection)}>Copy intro</button>{connection.peer?.discordUsername && <button type="button" onClick={() => void copyDiscordUsername(connection.peer.discordUsername)}>Copy Discord</button>}{connection.peer?.instagramUsername && <a href={`https://www.instagram.com/${encodeURIComponent(connection.peer.instagramUsername)}/`} target="_blank" rel="noreferrer">Open Instagram</a>}</div></article>)}</div></section>}
      {notice && <p className="buddy-notice" role="status">{notice}</p>}
      <div className="buddy-toolbar"><label><span>SEARCH PEOPLE</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, interest, major, Discord, or Instagram" /></label><p><b>{builders.length}</b><span>members available to discover</span></p></div>
      {status ? <p className="buddy-status">{status}</p> : matches.length ? <div className="buddy-grid">{matches.map(({ builder, shared }, index) => <article className="buddy-card" key={builder.uid}><div className="buddy-card-top"><b>{initials(builder.displayName)}</b><span>0{index + 1}</span></div><p className="buddy-card-label">{shared.length ? `${shared.length} shared interest${shared.length === 1 ? "" : "s"}` : "GDG KU builder"}</p><h3>{builder.displayName}</h3><p className="buddy-major">{builder.major || "GDG KU builder"}{builder.graduationYear ? ` · ${builder.graduationYear}` : ""}</p><div className="buddy-tags">{(shared.length ? shared : builder.interests.split(",").map((item) => item.trim()).filter(Boolean).slice(0, 3)).map((interest) => <span key={interest}>{interest}</span>)}</div><button className="buddy-view-pass" type="button" onClick={() => setSelectedBuilder(builder)}>View member card ↗</button><footer>{builder.leetCodeUsername ? <a href={`https://leetcode.com/u/${encodeURIComponent(builder.leetCodeUsername)}/`} target="_blank" rel="noreferrer">View LeetCode ↗</a> : <span>{builder.discordUsername || builder.instagramUsername ? "OPEN TO CONNECT" : "PROFILE COMPLETE"}</span>}<button type="button" disabled={sentTo.has(builder.uid)} onClick={() => void requestBuddy(builder)}>{sentTo.has(builder.uid) ? "Request sent" : "Connect"} <b>→</b></button></footer></article>)}</div> : <p className="buddy-status">No co-builders match this search yet. Ask another signed-in member to save their profile, then refresh.</p>}
      ...
      {selectedBuilder && <div className="member-pass-modal" role="dialog" aria-modal="true" aria-label={`${selectedBuilder.displayName}'s member card`} onClick={() => setSelectedBuilder(null)}><section onClick={(event) => event.stopPropagation()}><button onClick={() => setSelectedBuilder(null)} aria-label="Close member card">×</button><div className="member-pass-full"><p>GDG ON CAMPUS · KU</p><img className="member-pass-jayhawk" src="/assets/ku-jayhawk.png" alt="University of Kansas Jayhawk" /><i>COBUILDER MEMBER CARD</i><h2>{selectedBuilder.displayName}</h2><span>{selectedBuilder.major || "GDG KU builder"}</span><hr /><dl><div><dt>Graduation</dt><dd>{selectedBuilder.graduationYear ? `Class of ${selectedBuilder.graduationYear}` : "Not listed"}</dd></div><div><dt>LeetCode</dt><dd>{selectedBuilder.leetCodeUsername ? `@${selectedBuilder.leetCodeUsername}` : "Not listed"}</dd></div></dl><div className="member-pass-interests"><b>Interests</b><div>{selectedBuilder.interests.split(",").map((interest) => interest.trim()).filter(Boolean).slice(0, 4).map((interest) => <span key={interest}>{interest}</span>)}</div></div><footer><b>● GDG KU MEMBER</b><em>BUILD · LEARN · CONNECT</em></footer></div></section></div>}
    </section>
  </main>;
}
