...
export function CampusMarquee() {
  ...
  useEffect(() => {
    ...
    void import("three").then((THREE) => {
      ...
      const rings: Mesh[]=[]; [[2.0,.3,.9],[2.55,1.0,.2],[3.08,2.2,.7]].forEach(([r,x,y],i)=>{const ring=new THREE.Mesh(new THREE.TorusGeometry(r,.018,8,100),new THREE.MeshBasicMaterial({color:i===1?0xe51b3e:0xffffff,transparent:true,opacity:.72}));ring.rotation.set(x,y,i*.5);system.add(ring);rings.push(ring)});
