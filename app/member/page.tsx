...
function MemberPass({ member }: { member: GdgMember }) {
  ...
  return <main className={"member-page"}>
    <nav className={"member-nav"}><a href="/" className={"member-brand"}>GDG <span /> <em>ON CAMPUS<br />KU</em></a><div className={"member-nav-actions"}>{member.email.toLowerCase() === "heet2404@gmail.com" && <a className="member-admin-link" href="/admin/attendance">Attendance</a>}<MemberSignOut /></div><SectionMenu /></nav>
